...
public class PlayerPianoJ2ME {
	...
	static final String[] Chords = { "A ", "Ab ", "B ", "Bb ", "C ", "Cb ",
			"D ", "Db ", "E ", "Eb ", "F ", "Fb ", "G ", "Gb ", "> ", ". " };
	...
	public static byte[] PlayerPiano(String score) throws IOException {
		...
		while (n < notes.length) {
			...
			} else if (c == 's') { //NO SUCH funcitionality in MIDP, so we skip it
			//            sustain = !sustain;
	...
	public static void addNote(int startTick, int tickLength, int key,
			int velocity, ByteArrayOutputStream out) throws IOException {
		if (ticksSoFar < startTick) {
			out.write(new byte[] { ToneControl.SILENCE,
					(byte) (startTick - ticksSoFar) });
		}
		out.write(new byte[] { ToneControl.SET_VOLUME, (byte) velocity,
				(byte) key, (byte) tickLength });
