/**
 *  Filename: SelectLogModeForm.java (in org.openbandy.log)
...
/**
 * This form is used to set the level of filtering (i.e. what kind of log
 ...
 * @version 1.0
 */
public class SelectLogModeForm extends List implements BandyDisplayable, CommandListener {
	 ...
	 */
	public SelectLogModeForm() {
		super("Log Mode", Choice.IMPLICIT);
	...
	public void show(Display display, Displayable previousDisplayable) {
		...
		try {
			...
			for (int i = 0; i < logCanvas.config.modes.length; i++) {
				if (logCanvas.config.modes[i]) {
					append(LogConfiguration.modeLabels[i], imgChecked);
				}
				else {
					append(LogConfiguration.modeLabels[i], imgUnchecked);
				}
			}
	...
	public void commandAction(Command c, Displayable d) {
		if (c == cmdSelect) {
			try {
				...
				logCanvas.config.setLogMode(index, !logCanvas.config.modes[index]);
				if (logCanvas.config.modes[index]) {
					set(index, LogConfiguration.modeLabels[index], imgChecked);
				}
				else {
					set(index, LogConfiguration.modeLabels[index], imgUnchecked);
				}
			}
			catch (Exception e) {
