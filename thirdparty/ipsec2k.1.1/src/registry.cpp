...
bool x4_reg_key::open(HKEY hRootKey, const char * subkey, uint8 how, REGSAM regsam)
{
  ...
  if (how & x4c_reg_key_open)
	  if (ERROR_SUCCESS == RegOpenKeyEx(hRoot, to_wstring(path).c_str(), 0, sam, &hKey)) {
		  assert(hKey);
		  return true;
	  }

  if (how & x4c_reg_key_create)
    if (ERROR_SUCCESS == RegCreateKeyEx(hRoot, to_wstring(path).c_str(), 0, 0, 0, sam, 0, &hKey, &dwHow))
...
bool x4_reg_key::value_get(const char * label, vector<string> & v)
{
  ...
  {
    qos = temp.find((unsigned char) 0, pos);
    if (qos < 0)
    {
      v.clear();
...
bool x4_reg_key::remove(HKEY hRoot, const char * subkey)
{
  ...
  // $todo - enumerate and recursively remove all subkeys first
	return RegDeleteKey(hRoot, to_wstring(subkey).c_str()) == ERROR_SUCCESS;
}
...
{
	return RegDeleteValue(hKey, to_wstring(label).c_str()) == ERROR_SUCCESS;
}
...
bool x4_reg_key::read(const char * label, buffer & data, uint32 & type)
{
  ...
  size = 0;
  r = RegQueryValueEx(hKey, to_wstring(label).c_str(), 0, &type, 0, &size);
  if (r != ERROR_SUCCESS)
    return false;
  ...
  data.resize(size);
  r = RegQueryValueEx(hKey, to_wstring(label).c_str(), 0, &type, (BYTE*)data.data(), &size);
  if (r != ERROR_SUCCESS)
    return false;
...
bool x4_reg_key::write(const char * label, const void * data, uint32 size, uint32 type)
{
  ...
  r = RegSetValueEx(hKey, to_wstring(label).c_str(), 0, type, (BYTE*)data, size);
