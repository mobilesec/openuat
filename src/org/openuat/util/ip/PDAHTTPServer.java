 ...
 *  
 * <p>
 * NanoHTTPD version 1.05, Copyright &copy; 2001,2005,2006 Jarno Elonen
 ...
 * <li> File server does the 301 redirection trick for diectories without '/'</li>
 ...
 * licence)
 * 
 * added features by Relate Project
 * <ul>
...
public class PDAHTTPServer {
	 ...
	 */
	public PDAHTTPServer(int port, boolean deamon, InetAddress ia) throws IOException {
		myTcpPort = port;
		alive = true;
		listeners=new Vector();
		final ServerSocket ss = new ServerSocket(myTcpPort, 50, ia);
		logger.info("serversocket port: " +ss.getLocalPort());
		logger.info("server: address"+ ss.getInetAddress().getHostAddress());
	...
	public static void main(String[] args) {
		if (args.length!=2){
			System.out.println("Syntax error. Usage: java uk.ac.lancs.relate.ip NanoHTTPDGET 8080 true \n" +
					"Usage: java uk.ac.lancs.relate.ip NanoHTTPDGET [port] [asdeamon] \n");
			System.exit(0);
		}
		InetAddress ia = null;
		try {
			...
			if (System.getProperty("os.name").startsWith("Windows CE")){
				logger.info("Unknownn Host Exception: http Server not possible to set up");
				System.exit(0);
			}
			else{
			e.printStackTrace();}
	...
	private class HTTPSession implements Runnable {
		public HTTPSession(Socket s) {
			mySocket = s;
			logger.info("http session: client socket address: "+mySocket.getInetAddress());
		...
		public void run() {
			try {
				...
				if (method.equalsIgnoreCase("POST")) {
					...
					/*
					 * when the header includes the X-Relate-Filename, whe can
					 * be pretty sure that the data following it is a complete
					 * file. size of file is in default Content-Length header
					 * element, the file data itself are transmitted as bytes.
					 * (question to r gostner)
					 */
					String relateFile = header.getProperty(HTTPSocket.RELATE_ATTRIBUTE_FILENAME);
					...
					if (relateFile != null) {
						...
						for (int i = 0; i + b.length <= filesize; i = i
								+ b.length, counter++) {
							is.read(b);
							os.write(b);
							os.flush();
						}
						long rest = filesize - b.length * counter; 
						byte[] last = new byte[(int) rest];
						if (last.length != 0) {
							is.read(last);
							os.write(last);
							os.flush();
...
//					 Ok, now do the serve()
		...
		private String decodePercent(String str) throws InterruptedException {
			try {
				...
				sendAnswer(HTTP_BADREQUEST, "BAD REQUEST: Bad percent-encoding.");
				return null;
			}
		}
		 ...
		 */
		private void sendAnswer(String status, String msg)
				throws InterruptedException {
	...
	private String encodeUri(String uri) {
		...
		while (st.hasMoreTokens()) {
			...
			if (tok.equals("/"))
				...
				/*
				 * even if the method is deprcated, the other one
				 * URLEncoder.encode(tok, "UTF-8") is only since Java 1.4
				 */
