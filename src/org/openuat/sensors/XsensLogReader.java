...
public class XsensLogReader extends AsciiLineReaderBase {
	/** Our logger. */
	private static Logger logger = Logger.getLogger(XsensLogReader.class.getName());
	...
	protected void parseLine(String line) {
		...
		try {
			...
			logger.warning("Unable to decode sample number or timestamp, ignoring line: " + e);
			return;
		}
		...
		if (timestamp > lastSampleAt + SAMPLE_WIDTH) {
			logger.warning("Reading from the future (and jumping forwards by more than "
					+ SAMPLE_WIDTH + "ms: " +
		...
		for (int i=0; i<maxNumLines; i++) {
			try {
				...
				logger.warning("Unable to decode sample " + i);
				return;
			}
		}
