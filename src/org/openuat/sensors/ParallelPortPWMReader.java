...
public class ParallelPortPWMReader extends AsciiLineReaderBase {
	/** Our logger. */
	private static Logger logger = Logger.getLogger(ParallelPortPWMReader.class.getName());
	...
	protected void parseLine(String line) {
		...
		try {
			...
			logger.warning("Short line with incomplete timestamp, ignoring line");
			return;
		}
		catch (NumberFormatException e) {
			logger.warning("Unable to decode timestamp, ignoring line");
			return;
		}*/
		...
		if (logger.isLoggable(Level.FINER))
			logger.finer("Reading at timestamp " + timestamp + " us");
		// sanity check
		if (timestamp <= lastSampleAt) {
			logger.severe("Reading from the past (read " + timestamp + ", last sample at " + 
		...
		if (timestamp >= lastSampleAt + sampleWidth * 1000 &&
			...
			logger.severe("Reading from the future (and jumping forwards by more than 1000 samples, read " +
		...
		if (timestamp > lastSampleAt + sampleWidth) {
			if (logger.isLoggable(Level.FINER))
				logger.finer("Current reading creates new sample");
			...
			if (curSample[0].size() > 0) {
				if (logger.isLoggable(Level.FINER))
					logger.finer("Averaging over " + curSample[0].size() + " values for the last sample");
				for (int i=0; i<maxNumLines; i++) {
		...
		if (st.hasMoreElements()) {
			...
			while (elem<8 && st.hasMoreElements()) {
				try {
					...
					logger.warning("Unable to decode sample value, ignoring line");
					return;
				}
				elem++;
			}
			if (elem<8) {
				logger.warning("Short line, only got " + elem + " line values instead of " + 8 + ", ignoring line");
			}
			else {
				...
				for (int i=0; i<maxNumLines; i++) {
					int val = allLines[i]; 
					if (logger.isLoggable(Level.FINER))
						logger.finer("Read value " + val + " on line " + i);
			...
			if (logger.isLoggable(Level.FINER))
				logger.finer("This is an empty reading containing only a timestamp");
	}
	...
	public static void main(String[] args) throws IOException {
		org.openuat.sensors.test.AsciiLineReaderRunner.mainRunner("ParallelPortPWMReader", args);
	}
}
