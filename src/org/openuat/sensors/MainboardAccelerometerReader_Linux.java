...
public class MainboardAccelerometerReader_Linux extends AsciiLineReaderBase {
	...
	public MainboardAccelerometerReader_Linux(int samplerate) throws FileNotFoundException {
		...
		if (! System.getProperty("os.name").startsWith("Linux")) {
			logger.severe("Not running on Linux (os.name='" + System.getProperty("os.name") + "'), can not initialize!");
		...
		if (new File(applesmcSensorFile).canRead()) {
			...
			logger.severe("Detected neither hdaps nor applesmc sensor, maybe the kernel modules need to be loaded? Can not initialize!");
	...
	protected void parseLine(String line) {
		...
		if (line.charAt(0) != '(') {
			logger.severe("Can not decode line '" + line + "', expected it to start with '('");
			return;
		}
