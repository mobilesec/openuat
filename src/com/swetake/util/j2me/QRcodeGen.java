...
public QRcodeGen(Display display, Class appClass){
...
/* ---- read version ECC data file */
	...
	//InputStream fis =Qrcode.class.getResourceAsStream(filename);
	 InputStream fis = appClass.getResourceAsStream(filename);
...
//	    bis.close();
	 ...
	 System.out.println("rsEccCodewords: "+rsEccCodewords);
	fis.close();
    } catch(Exception e) {
        e.printStackTrace();
        log(e);
...
//        System.out.println("opening"+filename);
    ...
    while (rsBlockNumber<rsBlockOrder.length){
        byte[] rsTempData;
        rsTempData=clone(rsTemp[rsBlockNumber]);
