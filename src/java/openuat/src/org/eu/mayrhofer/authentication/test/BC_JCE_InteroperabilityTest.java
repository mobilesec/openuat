...
public class BC_JCE_InteroperabilityTest extends TestCase {
	...
	public void testDHAgreement() throws Exception {
		...
		for (int i=0; i<10; i++) {
			...
			kg2.init(new DHKeyGenerationParameters(new SecureRandom(), new DHParameters(skip1024Modulus, skip1024Base)));
			AsymmetricCipherKeyPair myKeypair2 = kg2.generateKeyPair();
			dh2.init(myKeypair2.getPrivate());
			
			// get JCE side public key
			byte[] publicKey1 = ((DHPublicKey) myKeypair1.getPublic()).getY().toByteArray();
			
			// get BC side public key
			byte[] publicKey2 = ((DHPublicKeyParameters) myKeypair2.getPublic()) .getY().toByteArray();
			
			// add other public key to JCE side
			DHPublicKey remotePublicKey1 = (DHPublicKey) java.security.KeyFactory.getInstance(
					KEYAGREEMENT_ALGORITHM).generatePublic(
					new DHPublicKeySpec(new BigInteger(publicKey2), skip1024Modulus, skip1024Base));
