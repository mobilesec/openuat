...
public class CandidateKeyProtocol {
	...
	public synchronized CandidateKey searchKey(Object remoteHost, byte[] hash, int [][] localIndices, int[][] remoteIndices) throws InternalApplicationException {
		...
		if (logger.isDebugEnabled()) {
			StringBuffer localTmp = new StringBuffer(), remoteTmp = new StringBuffer();
			for (int i=0; i<localIndices.length; i++) {
				localTmp.append(localIndices[i][0]);
				localTmp.append('/');
				localTmp.append(localIndices[i][1]);
				localTmp.append(' ');
				remoteTmp.append(remoteIndices[i][0]);
				remoteTmp.append('/');
				remoteTmp.append(remoteIndices[i][1]);
				remoteTmp.append(' ');
			}
			logger.debug("Trying to create key for remote '" + remoteHost + "' with hash " + 
